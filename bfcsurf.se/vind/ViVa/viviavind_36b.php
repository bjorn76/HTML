            ...
            if ( ! defined( self::get_config( 'script_id' ) ) ) {
                ...
                function resizeVivaNames(context = document) {
                    context.querySelectorAll('.viva-station-name').forEach(function(el) {
                        ...
                        let words = text.split(/\s+/); // Delar upp texten vid alla mellanslag
                        
                        if (words.length > 2) {
                            // Sätt in en <br> efter andra ordet
                            el.innerHTML = words[0] + ' ' + words[1] + '<br>' + words.slice(2).join(' ');
                        } else {
                            // Är namnet bara 1-2 ord, rör vi inget
                            el.innerHTML = text; 
                        }

                        // Tillåt radbrytning (skriver över "white-space: nowrap" från din PHP-kod)
                        el.style.whiteSpace = 'normal';
                        // Sätt en tajt radhöjd så att två rader får plats snyggt i rutan
                        el.style.lineHeight = '1.1';

                        // 2. Hantera fontstorlek: Max 28, Min 20
                        el.style.fontSize = '28px'; // Startstorlek
                        let size = 28;
                        
                        // Vi mäter nu både bredd och höjd, ifall de två raderna tar för mycket plats vertikalt
