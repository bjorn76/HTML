/**
 * Dynamisk ViVa Vind Data Shortcode med Auto-Refresh - Version 30
 * Uppdaterad för inline-flex beteende och automatisk radbrytning.
 * Usage: [vivavind_v30 id="220"]
 */
...
if ( ! class_exists( 'Viva_Wind_Widget_V30' ) ) {

    class Viva_Wind_Widget_V30 {

        // =========================================================================
        // CENTRALISERAD KONFIGURATION - VERSION 30
        // =========================================================================
        const VERSION     = '30';
        const SHORTCODE   = 'vivavind_v30'; 
        const CACHE_TTL   = 90; 
        ...
        private static function render_html( $data ) {
            ...
            ob_start();
            ?>
            <div style="display: inline-flex; flex-direction: column; align-items: center; vertical-align: top; min-width: 140px; box-sizing: border-box; border: 2px solid #000000; padding: 10px; margin: 5px; background-color: #f8fafc; font-family: sans-serif; text-align: center;">
                
                <h4 style="margin: 0 0 4px 0; font-size: 13px; color: #334155; font-weight: 600;"><?php echo $name; ?></h4>
                
                <div style="display: flex; flex-direction: column; align-items: center;">
                    <?php if ( $bywind_val ) : ?>
                        <div style="font-size: 11px; color: #64748b; font-weight: 500;"><?php echo esc_html( $bywind_val ); ?></div>
                    <?php endif; ?>

                    <div style="font-size: 20px; font-weight: bold; color: #0f172a; line-height: 1.1; margin: 2px 0;"><?php echo esc_html( $medelwind_val ); ?></div>
                    
                    <?php if ( 'N/A' !== $heading ) : ?>
                        <div style="font-size: 11px; color: #475569; font-weight: 500;"><?php echo $heading; ?></div>
                    <?php endif; ?>
                </div>

                <?php if ( 'Unknown' !== $updated ) : ?>
                    <div style="font-size: 9px; color: #94a3b8; margin-top: 6px;"><?php echo $updated; ?></div>
                <?php endif; ?>
        ...
        private static function get_widget_content( $station_id ) {
            ...
            if ( is_wp_error( $raw_data ) ) {
                return '<div style="display: inline-flex; justify-content: center; align-items: center; min-width: 140px; border: 2px solid #000000; padding: 10px; margin: 5px; box-sizing: border-box; font-size: 12px; color: #ef4444; text-align: center; font-family: sans-serif;">' . esc_html( $raw_data->get_error_message() ) . '</div>';
            }
