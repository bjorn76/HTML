/**
 * Dynamisk ViVa Vind Data Shortcode - Version 35
 * Fix: Automatisk skalning av stationsnamn (Auto-fit).
 * Usage: [vivavind_v35 id="220"]
 */
...
if ( ! class_exists( 'Viva_Wind_Widget_V35' ) ) {

    class Viva_Wind_Widget_V35 {

        const VERSION     = '35';
        const SHORTCODE   = 'vivavind_v35'; 
        const CACHE_TTL   = 90; 
        ...
        private static function format_data( $api_data ) {
            ...
            if (!empty($samples)) {
                ...
                // Use wind sample for heading + updated
                if ($firstWind) {
        ...
        private static function render_html( $data ) {
            ...
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 180px; height: 180px; border: 2px solid #cbd5e1; border-radius: 10px; padding: 10px; background: #f8fafc; font-family: sans-serif; text-align: center; box-sizing: border-box;">
                <!-- Namnet har nu en klass och en maxstorlek startpunkt -->
        ...
        public static function render_shortcode( $atts ) {
            ...
            if ( ! defined( self::get_config( 'script_id' ) ) ) {
                ...
                function resizeVivaNames() {
                    document.querySelectorAll('.viva-station-name').forEach(function(el) {
                        ...
                        document.querySelectorAll('.<?php echo esc_js( $wrapper_cl ); ?>').forEach(function(container) {
                            ...
                            .then(html => { 
                                if (html.trim() !== '') {
                                    ...
                                    resizeVivaNames(); // Kör om skalningen efter update
                                }
                            });
                        });
