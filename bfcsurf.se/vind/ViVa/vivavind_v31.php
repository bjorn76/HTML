/**
 * Dynamisk ViVa Vind Data Shortcode - Version 31
 * FIX: Wrapper-div satt till inline-block för att möjliggöra textflöde.
 * Usage: [vivavind_v31 id="220"]
 */
...
if ( ! class_exists( 'Viva_Wind_Widget_V31' ) ) {

    class Viva_Wind_Widget_V31 {

        const VERSION     = '31';
        const SHORTCODE   = 'vivavind_v31'; 
        const CACHE_TTL   = 90; 
        ...
        private static function render_html( $data ) {
            ...
            ob_start();
            ?>
            <div style="display: inline-flex; flex-direction: column; align-items: center; min-width: 130px; border: 2px solid #000; padding: 8px; background: #f8fafc; font-family: sans-serif; text-align: center;">
                <h4 style="margin: 0 0 4px 0; font-size: 12px; color: #334155;"><?php echo esc_html( $data['name'] ); ?></h4>
                <?php if ( $bywind_val ) : ?><div style="font-size: 10px; color: #64748b;"><?php echo esc_html( $bywind_val ); ?></div><?php endif; ?>
                <div style="font-size: 18px; font-weight: bold; color: #0f172a; margin: 2px 0;"><?php echo esc_html( $medelwind_val ?: 'Ingen data' ); ?></div>
                <?php if ( $data['heading'] !== 'N/A' ) : ?><div style="font-size: 10px; color: #475569;"><?php echo esc_html( $data['heading'] ); ?></div><?php endif; ?>
            </div>
            <?php
            return ob_get_clean();
        }
        ...
        public static function get_widget_content( $station_id ) {
            ...
            if ( is_wp_error( $data ) ) return '<div style="display:inline-block; border: 2px solid #000; padding: 10px; color: red;">Error</div>';
        ...
        public static function render_shortcode( $atts ) {
            ...
            ob_start();
            // VIKTIGT: display: inline-block + vertical-align: top gör att dessa nu beter sig som text
            echo '<div id="' . esc_attr( $unique_id ) . '" class="' . esc_attr( $wrapper_cl ) . '" data-station="' . $station_id . '" style="display: inline-block; vertical-align: top; margin: 3px;">';
