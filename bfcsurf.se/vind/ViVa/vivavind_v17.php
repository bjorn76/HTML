/** 
 * Dynamisk ViVa Vind Data Shortcode med Auto-Refresh - Version 17
 * Direkt översättning av Python-skriptets robusta metadata- och tvättlogik.
 * Usage: [vivavind_v17 id="220"]
 */
...
if ( ! class_exists( 'Viva_Wind_Widget_V17' ) ) {

    class Viva_Wind_Widget_V17 {

        // =========================================================================
        // CENTRALISERAD KONFIGURATION - ÄNDRA ENDAST HÄR VID NY VERSION
        // =========================================================================
        const VERSION     = '17';
        const SHORTCODE   = 'vivavind_v17'; 
        const CACHE_TTL   = 90; 
        ...
        /**
         * Datatvätt och strukturering - Exakt kopia av din Python-logik.
         */
        private static function format_data( $api_data ) {
            $formatted = [
                ...
                'heading'  => 'N/A',
                'updated'  => 'Unknown',
                'features' => [] // Här sparar vi alla mätvärden dynamiskt
            ];

            $samples = isset( $api_data['Samples'] ) ? $api_data['Samples'] : [];

            if ( ! empty( $samples ) ) {
                // 1. Extrahera delad metadata från första samplet (Exakt som Python)
                ...
                // Hitta bokstäver (riktning t.ex. "SO") via regex
                ...
                // 2. Processa och tvätta alla mätvärden dynamiskt (Exakt som Python-loopen)
                foreach ( $samples as $sample ) {
                    ...
                    // Python: re.sub(r'[^0-9.,-]', '', raw_val) -> Ersätt allt som INTE är siffror, punkter, komman eller minus
                    $clean_value = preg_replace( '/[^0-9.,-]/', '', $raw_val );
                    $clean_value = trim( $clean_value );

                    $value_with_unit = ! empty( $unit ) ? $clean_value . ' ' . $unit : $clean_value;

                    // Spara i en strukturerad lista för presentationen (nyckel-värde)
        ...
        /**
         * HTML/CSS-presentation (Kvadrat 180x180px med svart ram).
         * Sorterar dynamiskt Byvind överst och Medelvind under, precis som du ville ha det!
         */
        private static function render_html( $data ) {
            ...
            // Sortera ut värdena ur vår dynamiska lista baserat på dina designönskemål
            $bywind_val    = '';
            $medelwind_val = 'Ingen data';

            foreach ( $data['features'] as $key => $val ) {
                $low_key = strtolower( $key );
                if ( false !== strpos( $low_key, 'bywind' ) || false !== strpos( $low_key, 'vindby' ) ) {
                    $bywind_val = 'Byvind: ' . $val;
                }
                if ( false !== strpos( $low_key, 'medelwind' ) || false !== strpos( $low_key, 'hastighet' ) ) {
                    $medelwind_val = $val;
                }
            }
        ...
        /**
         * Kortkods-hanterare [vivavind_v17 id="..."]
         */
