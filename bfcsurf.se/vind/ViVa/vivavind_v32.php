/**
 * Dynamisk ViVa Vind Data Shortcode - Version 32
 * Uppdatering: Kvadratisk (180x180px), 10px rundade hörn, grå ram.
 * Responsiv font-skalning för stationsnamn.
 * Usage: [vivavind_v32 id="220"]
 */
...
if ( ! class_exists( 'Viva_Wind_Widget_V32' ) ) {

    class Viva_Wind_Widget_V32 {

        const VERSION     = '32';
        const SHORTCODE   = 'vivavind_v32'; 
        const CACHE_TTL   = 90; 
        ...
        private static function render_html( $data ) {
            ...
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 180px; height: 180px; border: 2px solid #cbd5e1; border-radius: 10px; padding: 10px; background: #f8fafc; font-family: sans-serif; text-align: center; box-sizing: border-box;">
                <h4 style="margin: 0; width: 100%; font-size: clamp(12px, 11%, 20px); color: #334155; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo esc_html( $data['name'] ); ?></h4>
                
                <div style="flex-grow: 1; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                    <?php if ( $bywind_val ) : ?><div style="font-size: 11px; color: #64748b;"><?php echo esc_html( $bywind_val ); ?></div><?php endif; ?>
                    <div style="font-size: 22px; font-weight: bold; color: #0f172a; margin: 4px 0;"><?php echo esc_html( $medelwind_val ?: 'Ingen data' ); ?></div>
                    <?php if ( $data['heading'] !== 'N/A' ) : ?><div style="font-size: 11px; color: #475569;"><?php echo esc_html( $data['heading'] ); ?></div><?php endif; ?>
                </div>

                <?php if ( $data['updated'] !== 'Unknown' ) : ?><div style="font-size: 9px; color: #94a3b8;"><?php echo esc_html( $data['updated'] ); ?></div><?php endif; ?>
            </div>
            <?php
            return ob_get_clean();
        }
        ...
        public static function get_widget_content( $station_id ) {
            ...
            if ( is_wp_error( $data ) ) return '<div style="display:flex; justify-content:center; align-items:center; width:180px; height:180px; border:2px solid #cbd5e1; border-radius:10px; padding:10px; color: #ef4444; font-size: 12px; font-family: sans-serif; text-align: center; box-sizing: border-box;">Fel vid hämtning</div>';
        ...
        public static function render_shortcode( $atts ) {
            ...
            ob_start();
            // Behåller inline-block + margin för att de ska flöda som text
